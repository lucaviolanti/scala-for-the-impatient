...
object Chapter09 {
  ...
  def searchAllButFPNumbers(pathToFile: String): Unit = {
    val file = Source.fromFile(pathToFile)
    val fpNumbers = "[0-9]+?\\.[0-9]+".r
    fpNumbers.findAllIn(file.mkString).foreach(println)
    file.close()
    // Try it with `searchAllButFPNumbers("src/main/resources/file-with-tabs.txt")`
  }

  // 8. Write a Scala program that prints the `src` attributes of all `img` tags of a web page.
  // Use regular expressions and groups.
  def searchImageSources(pathToFile: String): Unit = {
    val file = Source.fromFile(pathToFile)
    val sources = "<img.*src=(?:\"|')([^\"']+).*>".r
    sources.findAllIn(file.mkString).matchData.foreach(m => println(m.group(1)))
    file.close()
    // Try it with `searchImageSources("src/main/resources/sample.html")`
  }
